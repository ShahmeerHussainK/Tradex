...
import ManageTransactionsTableContainer from "./ManageTransactionTabelContainer";

export default function TransactionReport() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchedData, setSearchedData] = useState(TransactionsData);
  ...
  function handleSearchedData(currentSearch) {
    ...
    const searched = TransactionsData.tableRows.filter((row) => {
  ...
  return (
    <div className="space-y-10 px-6 py-12">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between sm:gap-0">
        <h3 className="text-xl font-bold text-gray-700">Transaction Report</h3>
      ...
      <div className="bg-white">
        <ManageTransactionsTableContainer
          data={
