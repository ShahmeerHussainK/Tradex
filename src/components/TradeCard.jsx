...
import { buyShare } from "@/api/matches";

export default function TradeCard({ latestData: { yes, no }, bet,handleEventDetail }) {
  const { id } = useParams();
  const [amount, setAmount] = useState(0);
  const [selectedBet, setSelectedBet] = useState(bet);

  const avgPrice = useMemo(()=>selectedBet === "yes" ? yes : selectedBet === "no" ? no : 0,[no, selectedBet, yes])
  ...
  function calculateShares(value) {
    return Number((amount / (value / 100)).toFixed(0));
  }
    ...
    if(amount){
      try{
        const payload ={
          user_id:'user1',
          event_id: id,
          outcome: selectedBet === 'yes'?1:0,
          shareCount:shares,
          share_price:avgPrice
        }
  ...
  },[amount, avgPrice, handleEventDetail, id, selectedBet, shares])
